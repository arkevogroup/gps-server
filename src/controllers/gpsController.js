...
import { createNewGpsValidator, createNewCommandValidator, sendCommandValidator } from "./validators/gpsApiValidator.js";
...
const createNewCommand = asyncHandler(async (req, res) =>{
    try {
        ...
        let response
        if ( (error.code ?? false) && error.code == "11000" ){
            response = {message :`Command: ${error.keyValue.command_name} already exists!`}
            res.status(400).json(response)
        }else{
            response = {message : error.details ? error.details[0].message : error.message }
            res.status(400).json(response)
...
const getAllCommands = asyncHandler(async (req, res) =>{
    try {
        const allCommands = await CommandModel.find({},{'createdAt':0, 'updatedAt':0, '__v':0});
...
const getAllAvailableDevices = asyncHandler(async (req, res) =>{
    try {
        const allDevices = await DeviceModel.find({},{'createdAt':0, 'updatedAt':0, '__v':0});
...
const sendCommandTeltonika = asyncHandler(async (req, res) =>{
    try {
        ...
        if( device && checkDeviceTeltonika ) {
            ...
            let response = {
                message : gpsDevice ?
                 `Can not send command to a device rather then Teltonika!` + ( device ? ' Device Provided: ' + device.protocol_name : '') : 
...
export { createNewGps, createNewCommand, getAllCommands, getAllAvailableDevices, sendCommandTeltonika };